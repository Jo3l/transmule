  ...
  <div :id="`page-provider-${providerId}`">
    ...
    <div v-if="showUrlBar" class="dt-url-bar mb-4">
      ...
      <SButton
        ...
        @click="saveAndLoad"
      >
        <span class="mdi mdi-content-save mr-1" />{{ $t("movies.saveUrl") }}
      </SButton>
    </div>
    ...
    <p v-else-if="!loading" class="has-text-muted">
      {{ mediaType === "shows" ? $t("shows.empty") : $t("movies.empty") }}
    </p>
        ...
        <div class="dt-modal">
          ...
          <div class="dt-modal-body">
            ...
            <div v-else-if="modal.item.links?.length" class="dt-modal-download">
              ...
              >
                <span class="mdi mdi-download mr-1" />{{ $t("movies.addToTransmission") }}
              </SButton>
            </div>
          </div>
        </div>
...
const mediaTypeLabel = computed(() =>
  mediaType.value === "shows" ? t("shows.title") : t("movies.title"),
);
...
async function onDownloadLink(item: MediaItem, link: MediaLink | null) {
  ...
  if (!link?.url && item.needsDetail) {
    ...
    try {
      ...
      if (!link?.url) {
        busy.value = null;
        error.value = mediaType.value === "shows" ? t("shows.noTorrent") : t("movies.noTorrent");
        return;
      }
    }
  }
...
async function load() {
  ...
  try {
    ...
    error.value =
      ...
      err?.message ||
      (mediaType.value === "shows" ? t("shows.fetchError") : t("movies.fetchError"));
  } finally {
    loading.value = false;
  }
}
